...
namespace KoleksiUmum\Controllers\Api;
...
class KoleksiUmum extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_koleksi_umum as a')
		...
		->addNumbering('no')
			->edit('title', function($row){
				...
				$html .= '<a href="'.permalink('koleksi/koleksi-umum/'.$row->slug).'" target="_blank">'.permalink('koleksi/koleksi-umum/'.$row->slug).'</a><br>';
				return $html;
			})
			->edit('file_cover', function($row){
				$default = base_url('uploads/default/no_cover.jpg');
				$image = (!empty($row->file_cover)) ? base_url('uploads/koleksiumum/' . $row->file_cover) : $default;
			...
			->edit('action', function($row){
				$edit = '<a href="'.base_url('cms/koleksiumum/edit/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="'.base_url('cms/koleksiumum/apply_status/'.$row->id.'?field=active&value=1').'"  data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="'.base_url('cms/koleksiumum/apply_status/'.$row->id.'?field=active&value=0').'" data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="'.base_url('cms/koleksiumum/delete/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
    ...
    {
		try {
            $data = $this->koleksiumumModel->where('slug', $slug)->first();
			if ($data) {
				$response = array(
					'error'    => false,
	...
	public function show($id = null)
    {
		try {
            ...
            $data = $this->koleksiumumModel->find($id);
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$save_data = array(
				...
				'id' => get_unique_id(),
				'category' => 'KoleksiUmum',
				'newsdate' => date('Y-m-d'),
				'images' => '220301083234ODMEqIWekT.jpg',
				'language' => 'id',
			);

			$id = $this->koleksiumumModel->insert($save_data);
			if ($id) {
				$data = $this->koleksiumumModel->find($id);
				$response = [
					'error'    => false,
	...
	public function update($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$koleksiumumUpdate = $this->koleksiumumModel->update($id, $update_data);
	...
	public function delete($id = null)
	{
		$data = $this->koleksiumumModel->find($id);
		if ($data) {
			$delete = $this->koleksiumumModel->delete($id);
			$response = [
				'error'    => false,
	...
	public function upload_file()
	{
        ...
        $koleksiumum= $this->koleksiumumModel->find($upload_id);
        $koleksiumumUpdate = $this->koleksiumumModel->update($upload_id,$update_data);
        if ($koleksiumumUpdate) {
			if($upload_field == 'file_image'){
				unlink_file($this->modulePath, $koleksiumum->file_image);
				unlink_file($this->modulePath, 'thumb_'.$koleksiumum->file_image);
			} else {
				unlink_file($this->modulePath, $koleksiumum->file_pdf);
			}	
