...
<div class="app-main__inner">
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>  Kamus
                    <div class="page-title-subheading">Daftar Semua  Kamus</div>
                </div>
            </div>
            <div class="page-title-actions">
                <nav class="" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="active breadcrumb-item" aria-current="page">  Kamus</li>
                    </ol>
                </nav>
            </div>
        </div>
	...
	<ul class="body-tabs body-tabs-layout tabs-animated body-tabs-animated nav">
		<li class="nav-item">
			<a class="nav-link <?=($slug == '')?'active':''?>" href="<?=base_url('cms/kamus?slug=')?>">
				<span>Semua</span>
			</a>
		</li>
		<?php foreach(get_ref('ref-kamus') as $row):?>
			<li class="nav-item">
				<a class="nav-link <?=($slug == slugify($row->name))?'active':''?>" href="<?=base_url('cms/kamus?slug='.slugify($row->name))?>">
					<span><?=$row->name?></span>
				</a>
			</li>
		<?php endforeach;?>
    ...
    <div class="main-card mb-3 card">
        <div class="card-header"><i class="header-icon lnr-list icon-gradient bg-plum-plate"> </i>Tabel  Kamus
            <div class="btn-actions-pane-right actions-icon-btn">
				<?php if(is_allowed('kamus/create')):?>
                <a href="<?= base_url('cms/kamus/create?slug='.$slug) ?>" class=" btn btn-success" title=""><i class="fa fa-plus"></i>
                    Tambah Kamus </a>
                <?php endif;?>
            </div>
        </div>
            ...
            <table style="width: 100%;" id="tbl_grid" class="table table-hover table-striped table-bordered">
                <thead>
                    <tr>
                        ...
                        <th class="text-center" width="">Judul Kamus</th>
...
<?= $this->section('script'); ?>
<?=$this->include('Kamus\Views\upload_modal');?>
<script>
	var t;
	$(document).ready(function() {
		t = $('#tbl_grid').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": '<?php echo site_url('api/kamus/datatable/'.$slug); ?>',
			},
			"dom": 
	...
	$("body").on("click", ".upload-data", function() {
		...
		var defaultDropzoneUrl = "<?= base_url('kamus/do_upload') ?>";
		var defaultUrl = "<?= base_url('api/kamus/upload_file') ?>";
		var defaultFormat = "application/pdf";
		var defaultFile = 1;
		var defaultRedirect = "<?= base_url('kamus') ?>";
		...
		setDropzone('file_pendukung', 'kamus', $('#upload_data_format').val(), $('#upload_data_file').val(), 10);
    });
