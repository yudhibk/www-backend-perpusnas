...
<div class="app-main__inner">
	<div class="app-page-title">
		<div class="page-title-wrapper">
          ...
          'auth'
						...
						<li class="breadcrumb-item"><a href="<?= base_url('survey') ?>">Survei </a></li>
						<li class="active breadcrumb-item" aria-current="page">Tambah Survei </li>
					</ol>
				</nav>
			</div>
		</div>
	...
	<div class="main-card mb-3 card">
		<div class="card-header">
			<i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> Form Tambah Survei
		</div>
		<div class="card-body">
								...
								<div class="form-group">
									...
									<div>
										<select class="form-control" name="surveyor_gender" id="surveyor_gender" tabindex="-1" aria-hidden="true">
											<?php foreach (get_ref('ref-jenis-kelamin', 'slug') as $row): ?>
								...
								<div class="form-group">
									...
									<div>
										<select class="form-control" name="surveyor_education" id="surveyor_education" tabindex="-1" aria-hidden="true">
											<?php foreach (get_ref('ref-pendidikan', 'slug') as $row): ?>
												<option value="<?= $row->name ?>"><?= $row->name ?></option>
											<?php endforeach; ?>
										</select>
									</div>
								...
								<div class="form-group">
									...
									<div>
										<select class="form-control" name="surveyor_job" id="surveyor_job" tabindex="-1" aria-hidden="true">
											<?php foreach (get_ref('ref-pekerjaan', 'slug') as $row): ?>
												<option value="<?= $row->name ?>"><?= $row->name ?></option>
											<?php endforeach; ?>
										</select>
									</div>
