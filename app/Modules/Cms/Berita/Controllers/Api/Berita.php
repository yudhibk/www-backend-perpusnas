...
class Berita extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_berita as a')
		...
		->addNumbering('no')
			->edit('title', function($row){
				...
				$html .= '<a href="'.permalink('publikasi/berita/'.$row->slug).'" target="_blank">'.permalink('publikasi/berita/'.$row->slug).'</a><br>';
				return $html;
			})
			...
			->edit('action', function($row){
				$edit = '<a href="'.base_url('cms/berita/edit/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="'.base_url('cms/berita/apply_status/'.$row->id.'?field=active&value=1').'"  data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="'.base_url('cms/berita/apply_status/'.$row->id.'?field=active&value=0').'" data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="'.base_url('cms/berita/delete/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
	...
	public function index($channel = null)
	{
		...
		$params['order'] = $params['order'] ?? 'publish_date';
		$params['direction'] = $params['direction'] ?? 'desc';
		$params['category'] = $params['category'] ?? '';
		$params['category_sub'] = $params['category_sub'] ?? '';
		...
		if(!empty($params['category_sub'])){
			$query->where('slugify(category_sub)',slugify($params['category_sub']));
		}
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$id = $this->beritaModel->insert($save_data);
			if ($id) {
				$data = $this->beritaModel->find($id);
				$response = [
					'error'    => false,
	...
	public function update($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$beritaUpdate = $this->beritaModel->update($id, $update_data);
			if ($beritaUpdate) {
				$data = $this->beritaModel->find($id);
				$response = [
					'error'    => false,
	...
	public function upload_file()
	{
        ...
        $berita= $this->beritaModel->find($upload_id);
        $beritaUpdate = $this->beritaModel->update($upload_id,$update_data);
        if ($beritaUpdate) {
			if($upload_field == 'file_image'){
