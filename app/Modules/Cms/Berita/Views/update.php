            ...
            <div class="page-title-actions">
                <nav class="" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="active breadcrumb-item" aria-current="page">Ubah Berita </li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="main-card mb-3 card">
            ...
            <div class="card-body">
                  ...
                  <form id="frm" class="col-md-12 mx-auto" method="post" action="">
                        <div class="form-row">
							<div class="col-md-12">
								<div class="position-relative form-group">
									...
									<div>
										<input required type="text" class="form-control" id="title" name="title" placeholder="Judul Berita" value="<?= set_value('title', $berita->title); ?>" />
										<small class="info help-block text-muted">Permalink: <a href="<?=permalink('publikasi/berita/'.$berita->slug)?>" target="_blank"><?=permalink('publikasi/berita/'.$berita->slug)?></a></small>
									</div>
								</div>
							</div>
							<div class="col-md-3">
								<div class="position-relative form-group">
									...
									<select class="form-control" name="category_sub" id="category_sub" tabindex="-1" aria-hidden="true">
										<?php foreach (get_ref('ref-berita','slug') as $row) : ?>
											<option value="<?= $row->name ?>" <?=($row->name == $berita->category_sub)?'selected':''?>><?= $row->name ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
							<div class="col-md-3">
								<div class="position-relative form-group">
									...
									<div>
										<input required type="date" class="form-control" id="publish_date" name="publish_date" placeholder="Tanggal Post" value="<?= set_value('publish_date', $berita->publish_date); ?>" />
									</div>
								</div>
							</div>
										...
										<div class="select-wrapper">
											<select class="form-control select2" name="channel" id="channel" tabindex="-1" aria-hidden="true" style="width:100%">
												...
												<?php foreach (get_ref_table('auth_groups', 'name,description', 'category="Unit Kerja"') as $row): ?>
													<option value="<?=$row->name?>" <?=($row->name == $berita->channel)?'selected':''?>><?=$row->description?></option>
												<?php endforeach;?>
											</select>
										</div>
									</div>
						...
						<div class="form-group">
                              <label for="content">Uraian Bahasa Inggris</label>
                              <div>
                                    <textarea id="content1" name="en_content" placeholder="" rows="1" class="form-control autosize-input"><?= set_value('en_content', $berita->en_content??''); ?></textarea>
                              </div>
                        </div>
						...
						<?php 
							...
							$file_cover = (!empty($berita->file_cover)) ? base_url('uploads/berita/' . $berita->file_cover) : $default;
						?>
						<div class="row">
							<div class="col-md-3">
								<div class="position-relative form-group">
						...
						<hr>
						<?php $file_images = array_filter(explode(',', $berita->file_image?? '')); ?>
								...
								<div class="position-relative form-group">
									...
									<div class="row">
										<?php foreach($file_images as $key => $value):?>
											<?php 
												$file_image = (!empty($value)) ? base_url('uploads/berita/' . $value) : $default;
											?>
											<div class="col-md-3">
												<div class="form-group">
													<div>
						...
						<?php if (is_admin()): ?>
							<?=$this->include('Berita\Views\meta_add');?>
						<?php endif;?>
...
<?= $this->section('script'); ?>
<?=$this->include('Berita\Views\meta_add_script');?>
<script>
	var file_cover = setDropzone('file_cover', 'cms/berita', '.png,.jpg,.jpeg', 1, 2);
	var max = '<?=$file_image_max - count($file_images)?>';
	var file_image = setDropzone('file_image', 'cms/berita', '.png,.jpg,.jpeg,.mp3,.mp4,.mkv,.pdf', max, 2);
</script>
<script>
	...
	$(document).ready(function() {
		<?php if(!empty($berita->meta)):?>
			<?php foreach(json_decode($berita->meta) as $row):?>
