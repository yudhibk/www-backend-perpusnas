...
<div class="app-main__inner">
      ...
      <div class="main-card mb-3 card">
            <div class="card-header">
                  <i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> Form Ubah Tentang
            </div>
            <div class="card-body">
                  <div id="infoMessage"><?= $message ?? ''; ?></div>
                  <?= get_message('message'); ?>

                  <form id="frm" class="col-md-12 mx-auto" method="post" action="<?= base_url('cms/tentang/edit/' . $tentang->id); ?>">
                        <div class="form-row">
                              <div class="col-md-12">
                                    <div class="position-relative form-group">
                                          <label for="name">Judul Tentang*</label>
                                          <div>
                                                <input type="text" class="form-control" id="frm_create_name" name="name" placeholder="Judul Tentang" value="<?= $tentang->name ?>" />
                                                <small class="info help-block text-muted">Judul Tentang</small>
                                          </div>
                                    </div>
                              </div>
                        </div>
                        ...
                        <div class="form-group">
                              ...
                              <div>
                                    <textarea id="frm_create_description" name="description" placeholder="Keterangan" rows="2" class="form-control autosize-input" style="min-height: 38px;"><?= $tentang->description ?></textarea>
                              </div>
                        </div>
...
<script>
      ...
      var dropzone_file = new Dropzone("#file", {
            ...
            url: "<?= base_url('cms/tentang/do_upload') ?>", // /do_uploads if multiple
            ...
            init: function() {
                  ...
                  var existingFile = "<?= $tentang->file ?>";
                  if (existingFile) {
                        var files = existingFile.split(',');
                        files.forEach(function(file) {
                              var uuid = Date.now();
                              var modulePath = "<?= base_url('uploads/tentang/') ?>";
            ...
            removedfile: function(file) {
                  ...
                  if (file.upload !== undefined) {
                        ...
                        name = file.name;
                        path = "<?= ROOTPATH . 'public/uploads/tentang/' ?>";
                  }

                  $.ajax({
                        type: 'POST',
                        url: "<?= base_url('cms/tentang/do_delete') ?>",
