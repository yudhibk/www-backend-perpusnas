...
namespace Pengumuman\Controllers\Api;
...
class Pengumuman extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_pengumuman as a')
		...
		$dataTable = DataTable::of($builder)
			...
			->edit('title', function ($row) {
				...
				$html .= '<a href="' . permalink('publikasi/pengumuman/' . $row->slug) . '" target="_blank">' . permalink('publikasi/pengumuman/' . $row->slug) . '</a><br>';
				return $html;
			})
			->edit('file_cover', function ($row) {
				$default = base_url('uploads/default/no_cover.jpg');
				$image = (!empty($row->file_cover)) ? base_url('uploads/berita/' . $row->file_cover) : $default;
			...
			->edit('action', function ($row) {
				$edit = '<a href="' . base_url('cms/pengumuman/edit/' . $row->id) . '" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="' . base_url('cms/pengumuman/apply_status/' . $row->id . '?field=active&value=1') . '"  data-id="' . $row->id . '" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="' . base_url('cms/pengumuman/apply_status/' . $row->id . '?field=active&value=0') . '" data-id="' . $row->id . '" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="' . base_url('cms/pengumuman/delete/' . $row->id) . '" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
	...
	{
		try {
			$data = $this->pengumumanModel->where('slug', $slug)->first();
			if ($data) {
				$response = array(
					'error'    => false,
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$id = $this->pengumumanModel->insert($save_data);
			if ($id) {
				$data = $this->pengumumanModel->find($id);
				$response = [
					'error'    => false,
	...
	public function update($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$pengumumanUpdate = $this->pengumumanModel->update($id, $update_data);
			if ($pengumumanUpdate) {
				$data = $this->pengumumanModel->find($id);
				$response = [
					'error'    => false,
	...
	public function upload_file()
	{
		...
		$pengumuman = $this->pengumumanModel->find($upload_id);
		$pengumumanUpdate = $this->pengumumanModel->update($upload_id, $update_data);
		if ($pengumumanUpdate) {
			if ($upload_field == 'file_image') {
				unlink_file($this->modulePath, $pengumuman->file_image);
				unlink_file($this->modulePath, 'thumb_' . $pengumuman->file_image);
			} else {
				unlink_file($this->modulePath, $pengumuman->file_pdf);
			}
