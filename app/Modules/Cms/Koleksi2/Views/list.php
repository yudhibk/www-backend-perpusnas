...
$model = new \Koleksi\Models\KoleksiModel();
$query = $model
	->select('t_koleksi.category_sub category, count(id) as total')
	->groupBy('t_koleksi.category_sub');

$categories = $query->orderBy('t_koleksi.category_sub', 'asc')->get()->getResult();
?>
...
<div class="app-main__inner">
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>  Koleksi
                    <div class="page-title-subheading">Daftar Semua  Koleksi</div>
                </div>
            </div>
            <div class="page-title-actions">
                <nav class="" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="active breadcrumb-item" aria-current="page">  Koleksi</li>
                    </ol>
                </nav>
            </div>
        </div>
	...
	<ul class="body-tabs body-tabs-layout tabs-animated body-tabs-animated nav">
		<li class="nav-item">
			<a class="nav-link <?=($slug == '')?'active':''?>" href="<?=base_url('cms/koleksi?slug=')?>">
				<span>Semua</span>
			</a>
		</li>
		<?php foreach($categories as $row):?>
			<li class="nav-item">
				<a class="nav-link <?=($slug == slugify($row->category))?'active':''?>" href="<?=base_url('cms/koleksi?slug='.slugify($row->category))?>">
    ...
    <div class="main-card mb-3 card">
        <div class="card-header"><i class="header-icon lnr-list icon-gradient bg-plum-plate"> </i>Tabel Koleksi
            <div class="btn-actions-pane-right actions-icon-btn">
				<?php if(is_allowed('koleksi/create')):?>
                <a href="<?= base_url('cms/koleksi/create?slug='.$slug) ?>" class=" btn btn-success" title=""><i class="fa fa-plus"></i>
                    Tambah Koleksi </a>
                <?php endif;?>
            </div>
        </div>
            ...
            <table style="width: 100%;" id="tbl_grid" class="table table-hover table-striped table-bordered">
                <thead>
                    <tr>
                        ...
                        <th class="text-center" width="">Judul Koleksi</th>
...
<?= $this->section('script'); ?>
<?=$this->include('Koleksi\Views\upload_modal');?>
<script>
	var t;
	$(document).ready(function() {
		t = $('#tbl_grid').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": '<?=site_url('api/koleksi/datatable/'.$slug)?>',
			},
			"dom": 
