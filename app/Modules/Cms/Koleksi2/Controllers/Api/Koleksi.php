...
namespace Koleksi\Controllers\Api;
...
class Koleksi extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_koleksi as a')
		...
		->addNumbering('no')
			->edit('title', function($row){
				...
				$html .= '<a href="'.permalink('publikasi/koleksi/'.$row->slug).'" target="_blank">'.permalink('publikasi/koleksi/'.$row->slug).'</a><br>';
				return $html;
			})
			->edit('file_cover', function($row){
				$default = base_url('uploads/default/no_cover.jpg');
				$image = (!empty($row->file_cover)) ? base_url('uploads/koleksi/' . $row->file_cover) : $default;
			...
			->edit('action', function($row){
				$edit = '<a href="'.base_url('cms/koleksi/edit/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="'.base_url('cms/koleksi/apply_status/'.$row->id.'?field=active&value=1').'"  data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="'.base_url('cms/koleksi/apply_status/'.$row->id.'?field=active&value=0').'" data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="'.base_url('cms/koleksi/delete/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
    ...
    {
		try {
            $data = $this->koleksiModel->where('slug', $slug)->first();
			if ($data) {
				$response = array(
					'error'    => false,
	...
	public function show($id = null)
    {
		try {
            ...
            $data = $this->koleksiModel->find($id);
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$id = $this->koleksiModel->insert($save_data);
			if ($id) {
				$data = $this->koleksiModel->find($id);
				$response = [
					'error'    => false,
	...
	public function update($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$koleksiUpdate = $this->koleksiModel->update($id, $update_data);
			if ($koleksiUpdate) {
				$data = $this->koleksiModel->find($id);
				$response = [
					'error'    => false,
	...
	public function delete($id = null)
	{
		$data = $this->koleksiModel->find($id);
		if ($data) {
			$delete = $this->koleksiModel->delete($id);
			$response = [
				'error'    => false,
	...
	public function upload_file()
	{
        ...
        $koleksi= $this->koleksiModel->find($upload_id);
        $koleksiUpdate = $this->koleksiModel->update($upload_id,$update_data);
        if ($koleksiUpdate) {
			if($upload_field == 'file_image'){
				unlink_file($this->modulePath, $koleksi->file_image);
				unlink_file($this->modulePath, 'thumb_'.$koleksi->file_image);
			} else {
				unlink_file($this->modulePath, $koleksi->file_pdf);
			}	
