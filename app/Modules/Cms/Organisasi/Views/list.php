...
<div class="app-main__inner">
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>Organisasi
                    <div class="page-title-subheading">Daftar semua Organisasi
                    </div>
                </div>
            </div>
	...
	<ul class="body-tabs body-tabs-layout tabs-animated body-tabs-animated nav">
		<li class="nav-item">
			<a class="nav-link <?=($slug == '')?'active':''?>" href="<?=base_url('cms/organisasi?slug=')?>">
				<span>Semua</span>
			</a>
		</li>
		<?php foreach(get_ref('ref-organisasi') as $row):?>
			<li class="nav-item">
				<a class="nav-link <?=($slug == slugify($row->name))?'active':''?>" href="<?=base_url('cms/organisasi?slug='.slugify($row->name))?>">
					<span><?=$row->name?></span>
				</a>
			</li>
		<?php endforeach;?>
    ...
    <div class="main-card mb-3 card">
        <div class="card-header"><i class="header-icon lnr-list icon-gradient bg-plum-plate">
            </i>Tabel Organisasi <?= ucwords(unslugify($slug))?>
            <div class="btn-actions-pane-right actions-icon-btn">
                <?php if(is_allowed('cms/organisasi/create')):?>
                <a href="<?= base_url('cms/organisasi/create?slug='.$slug) ?>" class=" btn btn-success" title=""><i class="fa fa-plus"></i>
                    Tambah Organisasi </a>
                <?php endif;?>
            </div>
        </div>
...
<script>
	...
	$(document).ready(function() {
		t = $('#tbl_grid').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": '<?php echo site_url('api/organisasi/datatable/'.$slug) ?>',
			},
			"dom": 
