...
<div class="app-main__inner">
	...
	<ul class="body-tabs body-tabs-layout tabs-animated body-tabs-animated nav">
		<li class="nav-item">
			<a class="nav-link <?=($slug == '')?'active':''?>" href="<?=base_url('cms/banner?slug=')?>">
				<span>Semua</span>
			</a>
		</li>
		<?php foreach(get_ref('ref-banner') as $row):?>
			<li class="nav-item">
				<a class="nav-link <?=($slug == slugify($row->name))?'active':''?>" href="<?=base_url('cms/banner?slug='.slugify($row->name))?>">
					<span><?=$row->name?></span>
				</a>
			</li>
		<?php endforeach;?>
    ...
    <div class="main-card mb-3 card">
        <div class="card-header"><i class="header-icon lnr-list icon-gradient bg-plum-plate">
            ...
            <div class="btn-actions-pane-right actions-icon-btn">
                <?php if(is_allowed('cms/banner/create')):?>
        ...
        <div class="card-body table-responsive">
            ...
            <table style="width: 100%;" id="tbl_grid" class="table table-hover table-striped table-bordered">
                <thead>
                    <tr>
                        <th width="35">No. </th>
                        <th width="100" class="text-center">Kategori / Channel</th>
                        <th width="80">Cover</th>
                        <th class="text-center">Judul Banner</th>
                        <th width="50">Urutan</th>
                        <th width="90">Highlight</th>
	...
	$(document).ready(function() {
		t = $('#tbl_grid').DataTable({
			...
			"columns": [
				...
				{data: 'category'},
				{data: 'file_image', className: 'text-center'},
				{data: 'title'},
