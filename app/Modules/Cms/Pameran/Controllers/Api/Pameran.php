...
class Pameran extends \App\Controllers\BaseResourceController
{
    use ResponseTrait;
    protected $client;
    ...
    public function datatable($slug = null)
    {
        $db = db_connect();
        $builder = $db
            ->table('t_pameran as a')
            ->select(
                'a.id, a.id as action, a.title, a.slug, a.file_cover, a.file_image, a.viewers, a.description, a.active'
            )
        ...
        $dataTable = DataTable::of($builder)
            ...
            ->edit('title', function ($row) {
                ...
                $html .=
                    '<a href="' .
                    permalink('publikasi/pameran/' . $row->slug) .
                    '" target="_blank">' .
                    permalink('publikasi/pameran/' . $row->slug) .
                    '</a><br>';
                return $html;
            ...
            ->edit('file_cover', function ($row) {
                ...
                $image = !empty($row->file_cover)
                    ? base_url('uploads/pameran/' . $row->file_cover)
                    : $default;

                $html =
                    '<a href="' .
                    $image .
                    '" class="image-link"><img width="100" class="rounded" src="' .
                    $default .
                    '" id="lazy' .
                    $row->id .
                    '" class="lazy" data-src="' .
            ...
            ->edit('category', function ($row) {
                $html =
                    ...
                    character_limiter($row->category, 15) .
                    '</span> ';
                if (!empty($row->category_sub)) {
                    $html .=
                        '<br><span class="badge badge-secondary badge-pill" >' .
                        $row->category_sub .
                        '</span>';
                }
                return $html;
            ...
            ->edit('action', function ($row) {
                $edit =
                    '<a href="' .
                    base_url('cms/pameran/edit/' . $row->id) .
                ...
                $delete =
                    ...
                    base_url('cms/pameran/delete/' . $row->id) .
    ...
    public function index($channel = null)
    {
        ...
        $query = $this->pameranModel->where('active', 1);
    ...
    {
        try {
            $data = $this->pameranModel->where('slug', $slug)->first();
            if ($data) {
                $response = [
                    'error' => false,
    ...
    {
        try {
            $data = $this->pameranModel->find($id);
    ...
    public function create()
    {
        ...
        if (
            ...
            $id = $this->pameranModel->insert($save_data);
            if ($id) {
                $data = $this->pameranModel->find($id);
                $response = [
                    'error' => false,
    ...
    public function update($id = null)
    {
        ...
        if (
            ...
            $pameranUpdate = $this->pameranModel->update($id, $update_data);
            if ($pameranUpdate) {
                $data = $this->pameranModel->find($id);
                $response = [
                    'error' => false,
    ...
    public function delete($id = null)
    {
        $data = $this->pameranModel->find($id);
        if ($data) {
            $delete = $this->pameranModel->delete($id);
            $response = [
                'error' => false,
    ...
    public function upload_file()
    {
        ...
        $pameran = $this->pameranModel->find($upload_id);
        $pameranUpdate = $this->pameranModel->update($upload_id, $update_data);
        if ($pameranUpdate) {
            if ($upload_field == 'file_image') {
