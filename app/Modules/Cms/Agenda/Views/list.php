...
<div class="app-main__inner">
	<div class="app-page-title">
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div> Agenda
					<div class="page-title-subheading">Daftar Semua Agenda</div>
				</div>
			</div>
			<div class="page-title-actions">
				<nav class="" aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="active breadcrumb-item" aria-current="page"> Agenda</li>
					</ol>
				</nav>
			</div>
		</div>
	...
	<ul class="body-tabs body-tabs-layout tabs-animated body-tabs-animated nav">
		<li class="nav-item">
			<a class="nav-link <?= ($slug == '') ? 'active' : '' ?>" href="<?= base_url('cms/agenda?slug=') ?>">
				<span>Semua</span>
			</a>
		</li>
		<?php foreach (get_ref('ref-agenda') as $row): ?>
			<li class="nav-item">
				<a class="nav-link <?= ($slug == slugify($row->name)) ? 'active' : '' ?>" href="<?= base_url('cms/agenda?slug=' . slugify($row->name)) ?>">
					<span><?= $row->name ?></span>
				</a>
			</li>
		<?php endforeach; ?>
	...
	<div class="main-card mb-3 card">
		<div class="card-header"><i class="header-icon lnr-list icon-gradient bg-plum-plate"> </i>Tabel Agenda
			<div class="btn-actions-pane-right actions-icon-btn">
				<?php if (is_allowed('cms/agenda/create')): ?>
					<a href="<?= base_url('cms/agenda/create?slug=' . $slug) ?>" class=" btn btn-success" title=""><i class="fa fa-plus"></i>
						Tambah Agenda </a>
				<?php endif; ?>
			</div>
		</div>
			...
			<table style="width: 100%;" id="tbl_grid" class="table table-hover table-striped table-bordered">
				<thead>
					<tr>
						...
						<th class="text-center" width="">Judul Agenda</th>
...
<?= $this->section('script'); ?>
<?= $this->include('Agenda\Views\upload_modal'); ?>
<script>
	var t;
	$(document).ready(function() {
		t = $('#tbl_grid').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": '<?= site_url('api/agenda/datatable/' . $slug) ?>',
			},
