...
namespace Publikasi\Controllers\Api;
...
class Publikasi extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_publikasi as a')
			->select('a.id, a.id as action, a.title, a.slug, a.file_cover, a.file_pdf, a.viewers, a.description, a.active')
		...
		->addNumbering('no')
			->edit('title', function($row){
				...
				$html .= '<a href="'.permalink('koleksi/buku-baru/'.$row->slug).'" target="_blank">'.permalink('koleksi/buku-baru/'.$row->slug).'</a><br>';
				return $html;
			})
			->edit('file_cover', function($row){
				$default = base_url('uploads/default/no_cover.jpg');
				$image = (!empty($row->file_cover)) ? base_url('uploads/publikasi/' . $row->file_cover) : $default;
			...
			->edit('action', function($row){
				$edit = '<a href="'.base_url('cms/publikasi/edit/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="'.base_url('cms/publikasi/apply_status/'.$row->id.'?field=active&value=1').'"  data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="'.base_url('cms/publikasi/apply_status/'.$row->id.'?field=active&value=0').'" data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="'.base_url('cms/publikasi/delete/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
    ...
    {
		try {
            $data = $this->publikasiModel->where('slug', $slug)->first();
			if ($data) {
				$response = array(
					'error'    => false,
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$id = $this->publikasiModel->insert($save_data);
			if ($id) {
				$data = $this->publikasiModel->find($id);
				$response = [
					'error'    => false,
	...
	public function update($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$publikasiUpdate = $this->publikasiModel->update($id, $update_data);
			if ($publikasiUpdate) {
				$data = $this->publikasiModel->find($id);
				$response = [
					'error'    => false,
	...
	public function delete($id = null)
	{
		$data = $this->publikasiModel->find($id);
		if ($data) {
			$delete = $this->publikasiModel->delete($id);
			$response = [
				'error'    => false,
	...
	public function upload_file()
	{
        ...
        $publikasi= $this->publikasiModel->find($upload_id);
        $publikasiUpdate = $this->publikasiModel->update($upload_id,$update_data);
        if ($publikasiUpdate) {
			if($upload_field == 'file_image'){
				unlink_file($this->modulePath, $publikasi->file_image);
				unlink_file($this->modulePath, 'thumb_'.$publikasi->file_image);
			} else {
				unlink_file($this->modulePath, $publikasi->file_pdf);
			}	
