...
namespace Testimoni\Controllers\Api;
...
class Testimoni extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_testimoni as a')
			->select('a.id, a.id as action, a.title, a.slug, a.content, a.file_cover, a.file_image, a.viewers, a.description, a.active')
		...
		->addNumbering('no')
			->edit('title', function($row){
				...
				$html .= '<a href="'.permalink('publikasi/testimoni/'.$row->slug).'" target="_blank">'.permalink('publikasi/testimoni/'.$row->slug).'</a><br>';
				return $html;
			})
			->edit('file_cover', function($row){
				$default = base_url('uploads/default/no_cover.jpg');
				$image = (!empty($row->file_cover)) ? base_url('uploads/testimoni/' . $row->file_cover) : $default;
			...
			->edit('action', function($row){
				$edit = '<a href="'.base_url('cms/testimoni/edit/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="'.base_url('cms/testimoni/apply_status/'.$row->id.'?field=active&value=1').'"  data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="'.base_url('cms/testimoni/apply_status/'.$row->id.'?field=active&value=0').'" data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="'.base_url('cms/testimoni/delete/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
    ...
    {
		try {
            $data = $this->testimoniModel->where('slug', $slug)->first();
			if ($data) {
				$response = array(
					'error'    => false,
	...
	public function show($id = null)
    {
		try {
            ...
            $data = $this->testimoniModel->find($id);
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$id = $this->testimoniModel->insert($save_data);
			if ($id) {
				$data = $this->testimoniModel->find($id);
				$response = [
					'error'    => false,
	...
	public function update($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$testimoniUpdate = $this->testimoniModel->update($id, $update_data);
			if ($testimoniUpdate) {
				$data = $this->testimoniModel->find($id);
				$response = [
					'error'    => false,
	...
	public function delete($id = null)
	{
		$data = $this->testimoniModel->find($id);
		if ($data) {
			$delete = $this->testimoniModel->delete($id);
			$response = [
				'error'    => false,
	...
	public function upload_file()
	{
        ...
        $testimoni= $this->testimoniModel->find($upload_id);
        $testimoniUpdate = $this->testimoniModel->update($upload_id,$update_data);
        if ($testimoniUpdate) {
			if($upload_field == 'file_image'){
				unlink_file($this->modulePath, $testimoni->file_image);
				unlink_file($this->modulePath, 'thumb_'.$testimoni->file_image);
			} else {
				unlink_file($this->modulePath, $testimoni->file_pdf);
			}	
