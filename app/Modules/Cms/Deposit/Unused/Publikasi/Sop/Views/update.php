...
<div class="app-main__inner">
      ...
      <div class="main-card mb-3 card">
            <div class="card-header">
                  <i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> Form Ubah Sop
            </div>
            <div class="card-body">
                  <div id="infoMessage"><?= $message ?? ''; ?></div>
                  <?= get_message('message'); ?>

                  <form id="frm" class="col-md-12 mx-auto" method="post" action="<?= base_url('deposit/publikasi/sop/edit/' . $sop->id); ?>">
                        <div class="form-row">
                              <div class="col-md-12">
                                    <div class="position-relative form-group">
                                          <label for="name">Judul Sop*</label>
                                          <div>
                                                <input type="text" class="form-control" id="frm_create_name" name="name" placeholder="Judul Sop" value="<?= $sop->name ?>" />
                                                <small class="info help-block text-muted">Judul Sop</small>
                                          </div>
                                    </div>
                              </div>
                        </div>
                        ...
                        <div class="form-group">
                              ...
                              <div>
                                    <textarea id="frm_create_description" name="description" placeholder="Keterangan" rows="2" class="form-control autosize-input" style="min-height: 38px;"><?= $sop->description ?></textarea>
                              </div>
                        </div>
...
<script>
      ...
      var dropzone_file_image = new Dropzone("#file", {
            ...
            url: "<?= base_url('deposit/publikasi/sop/do_upload') ?>", // /do_uploads if multiple
            ...
            init: function() {
                  ...
                  var existingFile = "<?= $sop->file ?>";
                  if (existingFile) {
                        var files = existingFile.split(',');
                        files.forEach(function(file) {
                              var uuid = Date.now();
                              var modulePath = "<?= base_url('uploads/deposit/publikasi/sop/') ?>";
            ...
            removedfile: function(file) {
                  ...
                  if (file.upload !== undefined) {
                        ...
                        name = file.name;
                        path = "<?= ROOTPATH . 'public/uploads/deposit/publikasi/sop/' ?>";
                  }

                  $.ajax({
                        type: 'POST',
                        url: "<?= base_url('deposit/publikasi/sop/do_delete') ?>",
