...
namespace Direktori\Controllers\Api;
...
class Direktori extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_direktori as a')
		...
		$dataTable = DataTable::of($builder)
			...
			->edit('category', function($row){
				$html = '<span class="badge badge-primary badge-pill">'.word_limiter($row->category,2).'</span> ';
				if(!empty($row->category_sub)){
					$html .= '<br><span class="badge badge-secondary badge-pill">'.word_limiter($row->category_sub,2).'</span>';
				}
				return $html;
			})
			...
			->edit('action', function($row){
				$edit = '<a href="'.base_url('cms/direktori/edit/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="'.base_url('cms/direktori/apply_status/'.$row->id.'?field=active&value=1').'"  data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="'.base_url('cms/direktori/apply_status/'.$row->id.'?field=active&value=0').'" data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="'.base_url('cms/direktori/delete/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
	...
	{	
		$db = db_connect();
		$builder = $db->table('t_direktori')
			->select('category, lower(REPLACE(category," ","-")) as category_slug')
			->distinct()
    ...
    {
		try {
            $data = $this->direktoriModel->where('slug', $slug)->first();
			if ($data) {
				$response = array(
					'error'    => false,
	...
	public function show($pk = null)
    {
		try {
            ...
            $data = $this->direktoriModel->find($pk);
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			if ($newDirektoriId) {
				$this->session->setFlashdata('toastr_msg', lang('Direktori.info.successfully_saved'));
				$this->session->setFlashdata('toastr_type', 'success');
				$response = [
					'status'   => 201,
					'error'    => false,
					'messages' => [
						'success' => lang('Direktori.info.successfully_saved')
					]
				];
				...
				$response = [
					...
					'error'    => false,
					'messages' => [
						'error' =>  lang('Direktori.info.failed_saved')
					]
				];
	...
	public function edit($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			if ($direktoriUpdate) {
				...
				$this->session->setFlashdata('toastr_msg', lang('Direktori.info.successfully_updated'));
				$this->session->setFlashdata('toastr_type', 'success');
				$response = [
					'status'   => 201,
					'error'    => false,
					'messages' => [
						'success' => lang('Direktori.info.successfully_updated')
					]
				];
				return $this->respond($response);
			} else {
				return $this->fail('<div class="alert alert-danger fade show" role="alert">'.lang('Direktori.info.failed_updated').'</div>', 400);
			}
		} else {
			$message = $this->validation->listErrors();
			return $this->fail($message, 400);
	...
	public function delete($id = null)
	{
		...
		$data = $this->direktoriModel->find($id);
		if ($data) {
			$this->direktoriModel->delete($id);
			add_log('Hapus Direktori', 'direktori', 'delete', 't_direktori', $id);
			$response = [
				'status'   => 200,
				'error'    => false,
				'messages' => [
					'success' => lang('Direktori.info.successfully_deleted')
				]
			];
			return $this->respondDeleted($response);
		} else {
			return $this->failNotFound(lang('Direktori.info.not_found').' ID:' . $id);
		}
	}
	...
	public function upload_file()
	{
        ...
        $direktori= $this->direktoriModel->find($upload_id);
        $direktoriUpdate = $this->direktoriModel->update($upload_id,$update_data);
        if ($direktoriUpdate) {
			if($upload_field == 'file_image'){
