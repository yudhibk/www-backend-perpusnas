...
$categories = $baseModel
      ...
      ->where('c_menus.name', 'Historidirektur')
      ->find_all('name', 'asc');
?>
...
<div class="app-main__inner">
                  ...
                  <div class="page-title-actions">
                        <nav class="" aria-label="breadcrumb">
                              <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="javascript:void(0)">Historidirektur</a></li>
      ...
      <div class="main-card mb-3 card">
            <div class="card-header">
                  <i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> Form Ubah Histori Direktur
            </div>
            <div class="card-body">
                  <div id="infoMessage"><?= $message ?? ''; ?></div>
                  <?= get_message('message'); ?>

                  <form id="frm" class="col-md-12 mx-auto" method="post" action="<?= base_url('cms/histori-direktur/edit/' . $historidirektur->id); ?>">
                        <div class="form-row">
                              <div class="col-md-12">
                                    <div class="position-relative form-group">
                                          <label for="name">Judul Historidirektur*</label>
                                          <div>
                                                <input type="text" class="form-control" id="frm_create_name" name="name" placeholder="Judul Historidirektur" value="<?= $historidirektur->name ?>" />
                                                <small class="info help-block text-muted">Judul Historidirektur</small>
                                          </div>
                                    </div>
                              </div>
                        </div>
                        ...
                        <div class="form-group">
                              ...
                              <div>
                                    <textarea id="frm_create_description" name="description" placeholder="Keterangan" rows="2" class="form-control autosize-input" style="min-height: 38px;"><?= $historidirektur->description ?></textarea>
                              </div>
                        </div>
...
<script>
      ...
      var dropzone_file = new Dropzone("#file", {
            ...
            url: "<?= base_url('cms/histori-direktur/do_upload') ?>", // /do_uploads if multiple
            ...
            init: function() {
                  ...
                  var existingFile = "<?= $historidirektur->file ?>";
                  if (existingFile) {
                        var files = existingFile.split(',');
                        files.forEach(function(file) {
                              var uuid = Date.now();
                              var modulePath = "<?= base_url('uploads/histori-direktur/') ?>";
            ...
            removedfile: function(file) {
                  ...
                  if (file.upload !== undefined) {
                        ...
                        name = file.name;
                        path = "<?= ROOTPATH . 'public/uploads/histori-direktur/' ?>";
                  }

                  $.ajax({
                        type: 'POST',
                        url: "<?= base_url('cms/histori-direktur/do_delete') ?>",
