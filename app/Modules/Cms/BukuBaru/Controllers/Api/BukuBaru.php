...
namespace BukuBaru\Controllers\Api;
...
class BukuBaru extends \App\Controllers\BaseResourceController
{
	use ResponseTrait;
	protected $client;
	...
	{
		$db = db_connect();
		$builder = $db->table('t_buku_baru as a')
		...
		->addNumbering('no')
			...
			->edit('file_cover', function($row){
				...
				$image = (!empty($row->file_cover)) ? base_url('uploads/bukubaru/' . $row->file_cover) : $default;
			...
			->edit('action', function($row){
				$edit = '<a href="'.base_url('cms/bukubaru/edit/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-primary show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
				$active = '<a href="'.base_url('cms/bukubaru/apply_status/'.$row->id.'?field=active&value=1').'"  data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Active" class="btn btn-success active-data"><i class="pe-7s-check font-weight-bold"> </i> </a>';
				$inactive = '<a href="'.base_url('cms/bukubaru/apply_status/'.$row->id.'?field=active&value=0').'" data-id="'.$row->id.'" data-toggle="tooltip" data-placement="top" title="Inactive" class="btn btn-warning draft-data"><i class="pe-7s-close font-weight-bold"> </i> </a>';
				$delete = '<a href="javascript:void(0);" data-href="'.base_url('cms/bukubaru/delete/'.$row->id).'" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
    ...
    {
		try {
            $data = $this->bukubaruModel->where('slug', $slug)->first();
			if ($data) {
				$response = array(
					'error'    => false,
	...
	public function show($id = null)
    {
		try {
            ...
            $data = $this->bukubaruModel->find($id);
	...
	public function create()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$id = $this->bukubaruModel->insert($save_data);
			if ($id) {
				$data = $this->bukubaruModel->find($id);
				$response = [
					'error'    => false,
	...
	public function update($id = null)
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$bukubaruUpdate = $this->bukubaruModel->update($id, $update_data);
			if ($bukubaruUpdate) {
				$data = $this->bukubaruModel->find($id);
				$response = [
					'error'    => false,
	...
	public function delete($id = null)
	{
		$data = $this->bukubaruModel->find($id);
		if ($data) {
			$delete = $this->bukubaruModel->delete($id);
			$response = [
				'error'    => false,
	...
	public function upload_file()
	{
        ...
        $bukubaru= $this->bukubaruModel->find($upload_id);
        $bukubaruUpdate = $this->bukubaruModel->update($upload_id,$update_data);
        if ($bukubaruUpdate) {
			if($upload_field == 'file_image'){
				unlink_file($this->modulePath, $bukubaru->file_image);
				unlink_file($this->modulePath, 'thumb_'.$bukubaru->file_image);
			} else {
				unlink_file($this->modulePath, $bukubaru->file_pdf);
			}	
