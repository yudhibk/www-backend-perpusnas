<?php
	...
	$file_image_max = 4;
	$file_images = array_filter(explode(',', $bukubaru->file_image?? '')); 
	$file_image_avail = $file_image_max - count($file_images);

	$default = base_url('uploads/default/no_cover.jpg'); 
	$file_cover = (!empty($bukubaru->file_cover)) ? base_url('uploads/bukubaru/' . $bukubaru->file_cover) : $default;
?>
...
<div class="app-main__inner">
    ...
    <div class="main-card mb-3 card">
		<div class="card-header">
			<i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> Form Ubah Buku Baru
		</div>
		<div class="card-body">
			...
			<form id="frm" class="col-md-12 mx-auto" method="post" action="">
				<div class="form-row">
					<div class="col-md-12">
						<div class="position-relative form-group">
							...
							<div>
								<input type="text" class="form-control" id="title" name="title" placeholder="Judul Buku Baru" value="<?=set_value('title', $bukubaru->title);?>" />
								<small class="info help-block text-muted">Permalink: <a href="<?=permalink('koleksi/buku-baru/' . $bukubaru->slug)?>" target="_blank"><?=permalink('koleksi/buku-baru/' . $bukubaru->slug)?></a></small>
							</div>
						</div>
					</div>
					<div class="col-md-3">
						<div class="position-relative form-group">
							...
							<select required class="form-control" name="category" id="category" tabindex="-1" aria-hidden="true">
								<?php foreach (get_ref('ref-bukubaru', 'slug') as $row): ?>
									<option value="<?=$row->name?>" <?=($row->name == $bukubaru->category) ? 'selected' : ''?>><?=$row->name?></option>
								<?php endforeach;?>
							</select>
						</div>
					</div>
					<div class="col-md-3">
						<div class="position-relative form-group">
							...
							<div>
								<input required type="date" class="form-control" id="publish_date" name="publish_date" placeholder="Tanggal Post" value="<?=set_value('publish_date', $bukubaru->publish_date);?>" />
							</div>
						</div>
					</div>
				</div>
				...
				<div class="form-row">
					<div class="col-md-3">
						<div class="position-relative form-group">
							...
							<div>
								<input type="number" class="form-control" id="year" name="year" placeholder="Tahun" value="<?=set_value('year', $bukubaru->year);?>" />
							</div>
						</div>
					</div>
					<div class="col-md-3">
						<div class="position-relative form-group">
							...
							<div>
								<input type="text" class="form-control" id="publisher" name="publisher" placeholder="Penerbit" value="<?=set_value('publisher', $bukubaru->publisher);?>" />
							</div>
						</div>
					</div>
					<div class="col-md-6">
						<div class="position-relative form-group">
							...
							<div>
								<input type="text" class="form-control" id="author" name="author" placeholder="Penulis" value="<?=set_value('author', $bukubaru->author);?>" />
							</div>
						</div>
					</div>
					<div class="col-md-12">
						<div class="position-relative form-group">
							...
							<div>
								<input type="text" class="form-control" id="source" name="source" placeholder="Sumber" value="<?=set_value('source', $bukubaru->source);?>" />
							</div>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
						...
						<div>
							<textarea id="description" name="description" placeholder="Keterangan" rows="2" class="form-control autosize-input" style="min-height: 38px;"><?=set_value('description', $bukubaru->description)?></textarea>
						</div>
				</div>
				...
				<div class="form-group">
						...
						<div>
							<textarea id="content" name="content" placeholder="" rows="1" class="form-control autosize-input"><?=set_value('content', $bukubaru->content);?></textarea>
						</div>
				</div>
				...
				<div class="row">
					<div class="col-md-3">
						<div class="position-relative form-group">
							<label for="file_cover" class="">Preview Cover Buku Baru</label>
							<div>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="position-relative form-group">
							<label for="file_cover" class="">Preview Galeri Buku Baru</label>
							<div class="row">
								<?php foreach ($file_images as $key => $value): ?>
									<?php $file_image = (!empty($value)) ? base_url('uploads/bukubaru/' . $value) : $default;?>
									<div class="col-md-3">
										<div class="form-group">
											<div>
...
<script>
	...
	var file_image = setDropzone('file_image', 'cms/bukubaru', '.png,.jpg,.jpeg', max, 2);
</script>
<script>
